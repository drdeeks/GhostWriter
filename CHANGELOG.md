...
### Known Issues (discovered, not yet fixed — see AGENTS.md "Known Issues")
- **`next` dependency regression:** pinned at `^9.3.3` (installed 9.5.5, released 2020, predates the App Router by 4 years — ships no `next/server` module at all). This is **not an intentional old pin** — git history proves `next` was correctly at `^16.1.4` as of commit `3a10141` (2026-01-24), then silently dragged back to `9.3.3` by commit `86f16ec` ("Pre-build state with approved scripts and security fixes", 2026-06-03), a large grab-bag commit consistent with a bad merge of a stale snapshot. Root cause of ~226 `tsc` errors across 40 files (`Cannot find module 'next/server'`) and most of the remaining critical/high CVEs in the dependency tree (loader-utils, tar, webpack, terser, postcss, chokidar all ship inside old Next's bundled tooling). Needs restoring to `^16.x`, verified against the current App Router codebase, not a blind `npm audit fix --force`.
- **32/45 jest suites fail** with `SyntaxError: Cannot use import statement outside a module` — confirmed pre-existing (reproduced identically on `HEAD~1` before any cleanup this session), likely from the two coexisting Babel configs (root `babel.config.js` vs. `jest.config.js`'s inline `next/babel`-based transform override). Not yet root-caused.
- `hardhat` has no upstream fix yet for its current audit advisory (`fixAvailable: false`).
