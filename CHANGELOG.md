...
### Changed
- **Git history rewritten (`main` only):** Reconciled three unreconciled author identities (`drdeeks@outlook.com`, GitHub noreply, an AWS EC2 fallback identity, plus a real `google-labs-jules[bot]` author) into a single canonical `drdeeks <drdeeks@drdeeks.xyz>` across all 155 pre-existing commits via `git-filter-repo` with a mailmap, and stripped all `Co-authored-by` trailers. Full commit count and history preserved, no squashing. Scoped to `main` only via `--refs main` — `legacy-main`, dependabot branches, and `origin` were untouched. A `pre-rewrite-backup-main` tag holds the pre-rewrite SHA for recovery.
- **Repo cleanup:** Deleted `new-branch` (confirmed stale — its `IMPLEMENTATION_SUMMARY.txt` predates fixes already present on `main`, e.g. the `withdrawRefund()` gas-griefing fix in `StoryManager.sol`). `main` confirmed as the correct/current branch.

### Removed
- **Dead/generated tracked files:** `coverage.json`, `playwright-report/`, `test-build/`, `scripts/deploy.js.backup2` (byte-identical duplicate of `deploy.js`), `package.minimal.json`, `jest.environment.js`, `jest.setup.cjs`, `jest.setup.ts`, `next-env.d.ts` (auto-regenerated by `next dev`/`next build`, should never have been tracked), `env.example` (duplicate of `.env.example`, content merged before deletion). All verified via full-tree grep/blob-hash comparison before removal — zero live references.
- **Unused legacy dependencies:** `present`, `script`, `scripts` — zero references anywhere in the codebase, unmaintained since ~2011-2013. `script` also chained to a critical `uglify-js` CVE and a high `detective` CVE via its own transitive tree.

### Fixed
- **`.gitignore` hardened:** removed inapplicable Gatsby/Nuxt/Storybook sections, fixed two live footguns (`CHANGELOG.md` and `public/` were both being ignored despite being tracked/wanted), deduped repeated patterns, added rules for `coverage.json`, `playwright-report/`, `test-build/`, `*.backup[0-9]*`.
- **Malformed `package-lock.json` entries:** 7 nested `@types/*` entries (`@types/ms`, `@types/istanbul-lib-coverage` x4, `@types/babel__traverse`) were bare `{}`/`{"dev": true}` stubs missing `version`/`resolved`/`integrity` — silently produced empty type-definition directories on every install and would break `npm ci` for any fresh clone. Root-caused and fixed via clean reinstall.
- **`npm audit` findings:** ran `npm audit fix` (non-breaking only, two passes to convergence) — 44 safe patch/minor bumps across the wallet/onchain stack (axios, ws, ethers, viem, wagmi, `@walletconnect/*`, `@reown/*`, `@metamask/*`). **113 → 99 known vulnerabilities** (critical 6→5, high 35→24).
- **Socket.dev CLI** set up for supply-chain scanning (`socket login`, org `drdeeks-unique-antiques`). First scan: 2056 packages, 62 high CVEs, 7 critical CVEs, 51 deprecated packages, 18 files with obfuscated-code signals, 1 troll-named package (`es5-ext`) flagged for review — full detail cross-referenced against `npm audit` rather than acted on blindly, since org security policy currently `ignore`s nearly everything (policy itself needs tightening separately).

### Fixed (later same day — see AGENTS.md "Known Issues" for detail)
- **`next` dependency regression resolved:** restored `next` from the regressed `^9.3.3` to `^16.1.4`, plus everything that had to move in lockstep — `react`/`react-dom` to `^19.2.3`, `@coinbase/onchainkit` to `1.1.2`, `wagmi` to `2.16.0` (onchainkit 1.1.2 peer-requires wagmi `^2.16`, not the v3 the regression had pulled forward), `tailwindcss` to `^4.1.16` (the CSS/postcss config was already v4-only syntax and would never have worked with the regressed v3 pin), and re-added `@farcaster/miniapp-sdk`/`@farcaster/miniapp-wagmi-connector`, which the same regression commit had dropped from `package.json` entirely despite 7 source files importing them.
- **Build toolchain repaired:** `next build`'s `--no-turbo` flag was renamed `--webpack` in Next 16; `babel.config.js` had a dead `importSource: '@emotion/react'` (package never installed, never used) and a CJS-forcing plugin that collided with Next's own ESM interop helpers in webpack server chunks; `browsers: 'last 2 versions'` browserslist target downleveled the `**` operator to `Math.pow()`, which throws on the `BigInt` operands viem/ethers use throughout.
- **Test toolchain repaired:** `jest.config.js`'s `next/babel` preset was silently failing to load (its compiled `{ default: fn }` export shape wasn't unwrappable by Babel's preset loader against this Next version), so every test file ran untransformed with zero warning. Replaced with the project's own preset set. Also wired the already-written but never-imported `src/test-utils/polyfills.ts` into `setup-tests.ts` (jsdom has no Fetch API), added a `viem/chains` mock mapping, gave `useReadContract`/`useWriteContract`/`useWaitForTransactionReceipt` mocks realistic default returns instead of bare `jest.fn()`, and fixed `hardhat.config.js`'s `paths.tests` (pointed at a nonexistent `./test` directory).
- **`@nomicfoundation/hardhat-network-helpers` restored** from `^3.0.9` (built for hardhat v3's plugin system) back to `^1.0.11`, paired with `hardhat-toolbox ^5.0.0` — the same regression commit had mismatched these, silently breaking `loadFixture` for every contract test.
- **Verified, not just claimed:** `npm run build` succeeds end-to-end (all 13 routes), `tsc --noEmit` is clean, `hardhat compile` + `test:contracts` pass 4/4. Jest: 278/317 passing (up from 150/164), with the remaining 39 failures confirmed as pre-existing application/test-content mismatches (e.g. a route test asserting an error message the route no longer returns) — not infrastructure failures.
