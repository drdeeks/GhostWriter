...
- **Keystore auto-generation**: If `STORY_TEMPLATE_SIGNER_PRIVATE_KEY` is not set in `.env`, the deploy script auto-generates a new random wallet, creates an encrypted keystore in `keystores/`, and sets it as the story template signer on-chain. The private key and keystore password are printed to console — **save them immediately**.
- **NFTs are NOT pre-generated**: NFTs are minted on-chain when users contribute words (`mintHiddenNFT`) or when stories complete (`mintCreatorNFT`). Metadata is dynamically generated by `/api/nft/[tokenId]/route.ts` based on on-chain data.
- After deployment, `deployment.json` is auto-updated with contract addresses and signer info. `.env` is also updated.
- `scripts/set-signer.js` reads from `.env` or `deployment.json` — no hardcoded addresses.
